<template>
  <aside class="helper">Scroll sideways</aside>
  <main>
    <figure class="rellax">
      <img
        src="https://i.postimg.cc/2j1K4rPC/evan-lee-3-CGcm-WDf-QNw-unsplash.jpg
"
        alt="Me coding"
      />
      <figcaption class="rellax" data-rellax-speed="1">
        Museo d'Arte Contemporanea
      </figcaption>
    </figure>
    <figure class="rellax">
      <img
        src="https://i.postimg.cc/yYqvc9mT/pavel-nekoranec-I-QKQLMIKs-unsplash.jpg
"
        alt="Bronte Beach, Australia"
      />
      <figcaption class="rellax" data-rellax-speed="1">
        National Etruscan Museum
      </figcaption>
    </figure>
    <figure class="rellax">
      <img
        src="https://i.postimg.cc/255wrxYC/milada-vigerova-Cuz-XP5e-FZb4-unsplash.jpg
"
      />
      <figcaption class="rellax" data-rellax-speed="1">
        Galleria Doria Pamphilj
      </figcaption>
    </figure>
  </main>
</template>
<script>
export default {};
</script>
<style scoped>
@import url("https://fonts.googleapis.com/css?family=Nunito+Sans|Playfair+Display:400,400i,700,700i,900,900i");

* {
  box-sizing: border-box;
  border-radius: 5px;
}

main {
  margin: 0;
  background-color: #eee;
  background-image: url(https://i.postimg.cc/T1R240BX/christopher-czermak-Pam-FFHL6f-VY-unsplash.jpg);
  background-size: cover;
}

main {
  min-height: 100vh;
  display: grid;
  grid-template-columns: 100vw 100vw 100vw;
  width: 300vw;
  justify-content: center;
  align-items: center;
}

figure {
  max-width: 50vw;
  margin: 0 auto;
  background: rgba(255, 255, 255, 0.8);
  padding: 2em;
  border: 1px solid #ddd;
  box-shadow: 0 0 2em -0.5em #aaa;
  position: relative;
}

figure img {
  max-width: 100%;
  max-height: 70vh;
}

figure figcaption {
  font-family: "Playfair Display";
  font-weight: 900;
  font-size: 3em;
  font-style: italic;
  text-align: right;
  text-shadow: 0 0 1em #555;
  text-shadow: 0 0 0.5em white;
  position: absolute;
  right: -2rem;
  bottom: -2rem;
}

.helper {
  position: absolute;
  top: 0;
  left: 0;
  margin: 1rem;
  text-align: center;
  font-family: "Nunito Sans";
  opacity: 0.45;
  font-size: 1.2em;
  transition: opacity, 0.15s;
}
.helper:hover {
  opacity: 1;
}
</style>